...
class PasswordManagerApp:
    def __init__(self, root):
        self.root = root
        self.root.title('AES Password Manager - MySQL')
    ...
    def show_login_screen(self):
        ...
        ttk.Label(frame, text='Master Password:').grid(row=2, column=0, sticky='w', pady=5)
    ...
    def show_main_screen(self):
        ...
        ttk.Button(top, text='Export Sync File', command=self.export_sync).pack(side='right', padx=5)
        ...
        form = ttk.LabelFrame(self.root, text='Shto / Përditëso fjalëkalim', padding=10)
    ...
    def generate_password(self): 
        ...
        self.password_entry.insert(0, generate_strong_password(16)) 

    def clear_form(self): 
        self.selected_id = None 
        for entry in [self.account_entry, self.username_entry, self.password_entry, self.notes_entry]: 
    ...
    def save_password(self): 
        ...
        if not account or not password: 
            messagebox.showerror('Gabim', 'Account dhe Password janë të detyrueshme.')
            return

        nonce, encrypted_password = encrypt_text(password, self.aes_key) 
        ...
        if self.selected_id: 
            cursor.execute('''
                UPDATE passwords
                SET account=%s, username=%s, category=%s, encrypted_password=%s, nonce=%s, notes=%s, updated_at=%s
                WHERE id=%s AND user_id=%s
            ''', (account, username, category, encrypted_password, nonce, notes, now, self.selected_id, self.user['id']))
        else:
            cursor.execute('''
                INSERT INTO passwords(user_id, account, username, category, encrypted_password, nonce, notes, updated_at)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
            ''', (self.user['id'], account, username, category, encrypted_password, nonce, notes, now))
        conn.commit() 
        cursor.close()
        conn.close()
